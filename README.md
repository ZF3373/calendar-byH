...
打开右侧 AI 面板 → 切到「AI教练」标签：

1. **生成计划**：在输入框填写目标，如 `帮我制定 ACM 竞赛 8 周备赛计划`，发送后 AI 会按天生成带真实日期的训练安排，解析为任务列表后勾选创建即可落到对应日历日。
2. **导入文档**：点「选择文档并解析」选 `.txt/.md` 文件。文档需按 `#### M月D日（周X）· 主题` 的小节标题组织（如训练计划 `1.md`），本地解析会按标题日期拆分任务并归属当天；无需经过 AI，结构不会丢失。
3. **相对周次文档**：若文档用「第 N 周 / 周 X」而非具体日期，解析后在「第1周从」填起始日期，任务按周次映射为绝对日期。
4. **清空导入**：重复导入前点「🗑 清空上次导入」，按 `AI 文档提取` 标记批量删除历史导入任务（不影响手动添加的任务）。

> 导入的任务统一带 `note: "AI 文档提取"`；创建后日历会自动跳转到首个任务日期，方便核对。
