...
- ✨ **AI 助手**：写周报、自然语言拆待办、对话问答；本地缓存优先，离线可用
- 🤖 **AI 教练（文档 / 计划导入）**：① 直接让 AI 生成训练 / 备赛计划（如「ACM 竞赛 8 周备赛」），按天排布真实日期；② 导入 `.txt/.md` 文档（如 `#### 7月17日（周五）· 综合练习` 结构），本地解析按日期拆分任务，自动落到对应日历日；③「🗑 清空上次导入」一键移除历史导入任务（不影响手动任务）
...
> 本机 Windows / MinGW 构建踩坑笔记：[DEV.md](DEV.md)
